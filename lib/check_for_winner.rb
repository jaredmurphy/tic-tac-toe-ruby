class CheckForWinner
    ...
    # property of the tile each tile has a value: 1 for 'X', 0 for blank tiles, and -1 for 'O'
    ...
    def check_for_horizontal(player = nil) # passes in the whole board array - rows
        check_rows(@board, player)
    end

    def check_for_vertical(player = nil) # passes in a 90 deg turned board - columns
        check_rows(@board.transpose, player)
    end

    def check_for_diagonal(player = nil) # passes the diagonals and reverse diagonals as a 2d array
        diagonal_range = (0..@board.length - 1)
        # credit to this post for getting diags - http://stackoverflow.com/a/2506734/4859818
    ...
    # when using this algorithm for a hint, we will just return the rows that are 1 play away from a win
    def check_rows(board, player = nil)
        ...
        if player # ie - if we are looking for a hint, not a winner
            board.select do |row|
                ...
                board_length_minus_one = (board.length) - 1
                ...
                row if (row_total == board_length_minus_one && row.include?(" ") && row.include?(player))
            end
        else # if we are looking for a winner
            board.each do |row|
                row_total = row.map(&tile_value).reduce(&:+).abs
                update_winner(row_total) if row_total == @board.length
            end
        end
    end
